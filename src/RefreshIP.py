...
#
# 功能：
#   通过 NetworkManager 刷新 IP 地址激活默认的网络连接
# 存在的问题：
#   1、许多 64 位 Linux 操作系统不支持 DBus 和 NetworkManager，这种情况下本脚本将无法正常运行
#   2、由于 sie-client 程序尚不支持无线网络，故此处已经跳过所有的无线网络配置
...
for connection_opath in active_connections:
	...
	if ac.settings['connection']['type'] == '802-3-ethernet': # 只找出有线以太网连接连接（因 sie-client 尚不支持无线）
